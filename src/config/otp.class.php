...
class OTP
{
    ...
    public function add($customerPhoneNumber)
    {
        $otp = rand(100000,999999);

        try {
            $this->connectDB();
            $sqlquery = "UPDATE Customers SET OTP=$otp WHERE PhoneNumber='$customerPhoneNumber'";
    ...
    public function read($customerPhoneNumber){

        try {
            $this->connectDB();
            $sqlquery = "SELECT OTP FROM Customers WHERE PhoneNumber=$customerPhoneNumber";
            $stmt = $this->connection->prepare($sqlquery);
            $this->connection->query("set character_set_client='utf8'");
            $this->connection->query("SET character_set_results = 'utf8', character_set_client = 'utf8', character_set_connection = 'utf8', character_set_database = 'utf8', character_set_server = 'utf8'");
            $stmt->execute();
